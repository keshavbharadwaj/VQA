...
def path_for(train=False, val=False, test=False, question=False, answer=False):
    ...
    s = fmt.format(config.get("task"), config.get("dataset"), split)
    return os.path.join(config.get("qa_path"), s)


def question_loading(questions_json):
    questions = [val["question"] for val in questions_json["questions"]]
    for question in questions:
        question = question.lower()
        yield question.split(" ")


def punction_remover(data):
    punctuation_pattern = "[\.\?\!\,\:\;\-\_\[\]\(\)\{\}'\"\%\*\#\@\^]+"
    message_cleaned = re.sub(punctuation_pattern, "", data)
    message_cleaned = message_cleaned.strip()
    return message_cleaned


def answer_loading(answers_json):
    answers = [
    ...
    for answer_list in answers:
        yield list(map(punction_remover, answer_list))
...
if __name__ == "__main__":
    ...
    with open(config.get("answer_train"), "r") as f:
        answers = json.load(f)
    print("questions format")
    pprint.pprint(questions["questions"][0])
    print("answers format")
    pprint.pprint(answers["annotations"][0])
    # answer_loading(answers)
